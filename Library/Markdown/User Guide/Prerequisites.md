...
Prerequisites appear on the **Detail Editor** of [Traits](Traits), [Skills](Skills), [Spells](Spells), and
[Equipment](Equipment). They define the requirements your character must meet before taking or using the item.

## Unmet prerequisites

If you add something to your [character sheet](Character%20Sheet%20Overview) without meeting its requirements, GCS shows
an error message below it.

![unmet prereqs](images/screenshots/scrn-unmetPrerequisite.png)
...
2. In the Prerequisite list, select **Add** ![add](images/icons/icn-add.svg).

To add a sub-list at a specific level:

- Select **Add child** ![add child](images/icons/icn-addChild.svg) next to that prerequisite.
...
> [!WARNING]
> When typing names (traits, skills, spells, equipment), you must match the name **exactly** as it appears in the
> library or on your sheet. Typos or mismatches mean the prerequisite won’t be recognized.
...
Targets define what the prerequisite applies to. These appear in dropdowns when you choose the requirement type. The
fields you see depend on the target you are selecting.
...
- **Name**: Compare the trait name (_is_, _is not_, _contains_, _starts with_, _ends with_). Enter the name in the text
  field.
...
Requires a custom script to check the prerequisite. See the scripting guide ![scripting
guide](images/icons/icn-scriptingGuide.svg) for more details.
...
> [!IMPORTANT]
> Scripts are evaluated much more slowly than other prerequisite types. Use this option only when no other type can
...
> [!TIP]
> Use sub-lists when you want **multiple ways** to qualify for something, or when a rule requires **several conditions
> at once.**

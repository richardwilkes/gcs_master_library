...
tags to [Traits](Traits), [Skills](Skills), [Spells](Spells), and [Equipment](Equipment).
...
- Select a tag to show only items that match. Select and hold `Shift` or `Ctrl` on your keyboard to select multiple
  tags.
...
- **Equipment:** Tag by type or use, such as `Weapon`, `Shield`, `Armor`, `Medical Gear`, `Ammunition`, or `Tools`.
  - Some equipment also uses tags for identify which ammo the weapon uses, like `AmmoType:Arrow` or
    `UsesAmmoType:Sling`.
